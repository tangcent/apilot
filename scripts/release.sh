...
release_branch="cli-release/v${next_version}"
git branch "${release_branch}"
git checkout "${release_branch}"

npm pkg set version="${next_version}"
echo "Updated version in package.json to ${next_version}"
...
commit_message="release: cli v${next_version}"
...
echo "  3. On main push with 'release: cli' in commit message to trigger CLI Release"
